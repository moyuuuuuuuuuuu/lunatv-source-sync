...
const result = ref<{items:Source[];total:number;page:number;pageSize:number}>({ items:[], total:0, page:1, pageSize:25 });
...
<template>
  <section class="sources-page">
    ...
    <div class="filters panel"><label><span class="sr-only">搜索</span><input v-model="filters.search" type="search" placeholder="搜索名称、键或 API"></label><select v-model="filters.classification" aria-label="分类"><option value="">全部分类</option><option value="normal">普通</option><option value="adult">成人</option></select><select v-model="filters.healthStatus" aria-label="健康状态"><option value="">全部状态</option><option value="healthy">健康</option><option value="unhealthy">异常</option><option value="unknown">未检测</option></select><select v-model="filters.enabled" aria-label="启用状态"><option value="">全部启用状态</option><option value="true">已启用</option><option value="false">已停用</option></select></div>
    ...
    <div class="table-wrap sources-table-wrap panel"><table><thead><tr><th><input type="checkbox" :checked="allSelected" aria-label="选择本页全部" @change="toggleAll"></th><th>来源</th><th>分类</th><th>状态</th><th>启用</th><th>操作</th></tr></thead><tbody><tr v-if="busy&&!result.items.length"><td colspan="6">载入中…</td></tr><tr v-else-if="!result.items.length"><td colspan="6" class="empty">没有符合条件的来源</td></tr><tr v-for="source in result.items" :key="source.id"><td><input v-model="selected" type="checkbox" :value="source.id" :aria-label="`选择 ${source.name}`"></td><td><b>{{source.name}}</b><code>{{source.sourceKey}}</code><small class="truncate" :title="source.api">{{source.api}}</small></td><td><span class="tag">{{source.isAdult?'成人':'普通'}}</span></td><td><span :class="['status',source.healthStatus]">{{source.healthStatus==='healthy'?'健康':source.healthStatus==='unhealthy'?'异常':'未检测'}}</span><small v-if="source.latencyMs!==null">{{source.latencyMs}} ms</small></td><td>{{source.enabled?'是':'否'}}</td><td><div class="row-actions"><button class="link" @click="editor=source">编辑</button><button class="link" @click="check(source)">检测</button><button class="link danger" @click="remove(source)">删除</button></div></td></tr></tbody></table></div>
