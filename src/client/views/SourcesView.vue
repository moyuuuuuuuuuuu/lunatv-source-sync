...
interface Source { id:number; sourceKey:string; name:string; api:string; sourceType:'vod_api'|'live_m3u'|'tvbox'|'navigation'; contentCategory:'general'|'movie'|'short_drama'; classificationMode:'auto'|'normal'|'adult'; isAdult:boolean; enabled:boolean; ignoreHealthCheck:boolean; healthStatus:'unknown'|'healthy'|'unhealthy'; latencyMs:number|null; lastCheckedAt:string|null; detail:string|null; comment:string|null }
const result = ref<{items:Source[];total:number;page:number;pageSize:number}>({ items:[], total:0, page:1, pageSize:10 });
const filters = reactive({ search:'', sourceType:'', contentCategory:'', classification:'', healthStatus:'', enabled:'' });
...
async function check(source:Source) { try{const response=await api.send<{result?:{status:string;errorMessage?:string|null}}>(`/api/admin/sources/${source.id}/check`,'POST');await load();if(response.result?.status==='unhealthy')error.value=`检测异常：${response.result.errorMessage||'上游响应无效'}`}catch(cause){error.value=cause instanceof Error?cause.message:'检测失败'} }
...
async function removeDuplicatesNow(){busy.value=true;error.value='';try{const response=await api.send<{affected:number;groups:number}>('/api/admin/sources/remove-duplicates','POST');await load(1);error.value=response.affected?`已移除 ${response.affected} 个重复来源（${response.groups} 组）`:'未发现重复 API 地址'}catch(cause){error.value=cause instanceof Error?cause.message:'去重失败'}finally{busy.value=false}}
async function removeDuplicates(){confirmation.value={title:'检测并移除重复资源',description:'系统将按规范化 API 地址检测重复项，每组保留最早添加的一条，其余重复来源将被删除。此操作无法恢复。',confirmText:'检测并移除',run:removeDuplicatesNow}}
...
<template>
  <section class="sources-page">
    <div class="page-head"><div><div class="eyebrow">SOURCES</div><h1>视频源管理</h1><p class="muted">共 {{result.total}} 个来源</p></div><div class="actions"><button class="ghost danger-button" :disabled="busy" @click="removeUnhealthy">移除异常</button><button class="ghost" :disabled="busy" @click="removeDuplicates">检测并移除重复</button><button class="ghost" @click="showImport=true">导入视频源</button><button class="primary" @click="editor=null">＋ 添加视频源</button></div></div>
    <div class="filters panel"><label><span class="sr-only">搜索</span><input v-model="filters.search" type="search" placeholder="搜索名称、键或地址"></label><select v-model="filters.sourceType" aria-label="来源类型"><option value="">全部类型</option><option value="vod_api">影视点播 API</option><option value="live_m3u">直播 M3U</option><option value="tvbox">TVBox 配置</option><option value="navigation">配置导航</option></select><select v-model="filters.contentCategory" aria-label="内容分类"><option value="">全部内容</option><option value="movie">影视</option><option value="short_drama">短剧</option><option value="general">综合</option></select><select v-model="filters.classification" aria-label="分级"><option value="">全部分级</option><option value="normal">普通</option><option value="adult">成人</option></select><select v-model="filters.healthStatus" aria-label="健康状态"><option value="">全部状态</option><option value="healthy">健康</option><option value="unhealthy">异常</option><option value="unknown">未检测</option></select><select v-model="filters.enabled" aria-label="启用状态"><option value="">全部启用状态</option><option value="true">已启用</option><option value="false">已停用</option></select><select v-model="latencySort" aria-label="响应速度排序"><option value="">默认排序</option><option value="latencyAsc">速度从快到慢</option><option value="latencyDesc">速度从慢到快</option></select></div>
    ...
    <div class="table-wrap sources-table-wrap panel"><table><thead><tr><th><input type="checkbox" :checked="allSelected" aria-label="选择本页全部" @change="toggleAll"></th><th>来源</th><th>类型 / 分类</th><th>状态</th><th>启用</th><th>操作</th></tr></thead><tbody><tr v-if="busy&&!result.items.length"><td colspan="6">载入中…</td></tr><tr v-else-if="!result.items.length"><td colspan="6" class="empty">没有符合条件的来源</td></tr><tr v-for="source in result.items" :key="source.id"><td><input v-model="selected" type="checkbox" :value="source.id" :aria-label="`选择 ${source.name}`"></td><td><b>{{source.name}}</b><code>{{source.sourceKey}}</code><a class="api-link truncate" :href="source.api" target="_blank" rel="noopener noreferrer" :title="`在新标签页打开 ${source.api}`"><span>{{source.api}}</span><i aria-hidden="true">↗</i></a><small :class="['source-latency',source.latencyMs===null?'unknown':source.latencyMs<1000?'fast':source.latencyMs<3000?'medium':'slow']">响应 {{source.latencyMs===null?'未检测':`${source.latencyMs} ms`}}</small></td><td><span class="tag">{{source.sourceType==='vod_api'?'点播 API':source.sourceType==='live_m3u'?'直播 M3U':source.sourceType==='tvbox'?'TVBox':'导航'}}</span><small>{{source.contentCategory==='short_drama'?'短剧':source.contentCategory==='movie'?'影视':'综合'}} · {{source.isAdult?'成人':'普通'}}</small></td><td><span :class="['status',source.healthStatus]">{{source.healthStatus==='healthy'?'健康':source.healthStatus==='unhealthy'?'异常':'未检测'}}</span></td><td>{{source.enabled?'是':'否'}}</td><td><div class="row-actions"><button class="link" @click="editor=source">编辑</button><button class="link" @click="check(source)">检测</button><button class="link danger" @click="remove(source)">删除</button></div></td></tr></tbody></table></div>
