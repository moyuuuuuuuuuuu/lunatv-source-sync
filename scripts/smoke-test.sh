...
export SUBSCRIPTION_TOKEN="smoke-subscription-token"
...
curl -fsS "$BASE_URL/api/source?ac=list&source=normal&format=json&proxy=0&token=smoke-subscription-token" >"$SMOKE_TMP/source.json"
...
curl -fsS "$BASE_URL/api/source?ac=list&source=normal&format=base58&proxy=0&token=smoke-subscription-token" >"$SMOKE_TMP/source.base58"
...
[ "$(curl -sS -o /dev/null -w '%{http_code}' "$BASE_URL/api/source?source=invalid&token=smoke-subscription-token")" = 400 ]
[ "$(curl -sS -o /dev/null -w '%{http_code}' "$BASE_URL/api/source?format=invalid&token=smoke-subscription-token")" = 400 ]
