...
- 本项目仅用于软件开发、网络技术研究与学习交流，不提供、存储或分发任何影视内容，也不代表对任何第三方内容、接口或服务的推荐、认可或授权。
- 用户导入的视频源、订阅地址及其返回内容均来自用户自行配置的第三方服务。本项目无法控制其合法性、准确性、安全性、稳定性和持续可用性，相关责任由内容提供方及实际使用者承担。
- 使用者应遵守所在地适用的法律法规、版权规则、网络服务条款及第三方接口约定，不得将本项目用于侵权、违法传播、绕过访问控制或其他未经授权的用途。
- 本项目按“现状”提供，不对功能完整性、适销性、特定用途适用性、数据安全或服务连续性作任何明示或默示保证。因部署、配置、使用或无法使用本项目造成的数据丢失、服务中断或其他损失，使用者应自行评估并承担风险。
- 若相关权利人认为通过本项目配置的第三方来源侵犯了其合法权益，应优先联系对应来源的运营者或实际部署者处理。本项目维护者不托管第三方内容，也无法管理各独立部署实例。

继续下载、部署或使用本项目，即表示使用者已阅读并理解上述说明。该免责声明不构成法律意见；如对具体使用场景的合规性存在疑问，请咨询具备相应资质的专业人士。
